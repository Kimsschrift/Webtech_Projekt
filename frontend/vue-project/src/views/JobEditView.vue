    ...
    axios.get(`/api/jobpostings/${id}`)
        .then(res => {
          ...
          this.form = {
            ...
            location: p.location,
            remoteAllowed: p.remoteAllowed,
            requiresExperience: p.requiresExperience,
          ...
          this.languages = p.languages.join(', ')
        })
        .catch(() => alert('Job konnte nicht geladen werden'))
  },
  methods: {
    async submit() {
      ...
      try {
        await axios.put(`/api/jobpostings/${this.$route.params.id}`, payload)
